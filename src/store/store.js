...
import poConfirmationRedcuer from "./reducers/poConfirmationReducer";
...
function initStore(hintText, connectingText, socket, storage, docViewer = false) {
        ...
        switch (action.type) {
            ...
            case actionTypes.SEND_INDENT_DATA: {
                ...
                if (store.getState().indents.indentData.length > 1) {
                    ...
                    action.data = "/reply_ask_more_indent_items";
                }
            ...
            case actionTypes.SEND_AGAINST_SAMPLE_INDENT_DATA: {
                action.customData = action.customData.map(data => ({
                    pd_item_dt_id: data.PD_ITEM_DT_ID,
                    pd_id: data.PD_ID,
                    item_id: data.ITEM_ID,
                    uom_id: data.UOM_ID,
                    quantity_req: data.QUANTITY,
            ...
            case actionTypes.SEND_PO_COFIRMATION_DATA: {
                console.log(
                    "*************************sending po confirmation data to server ......",
                    action.data
                );
                console.log(action.type);
                socket.emit("user_uttered", {
    ...
    const reducer = combineReducers({
        ...
        poConfirmation: poConfirmationRedcuer()
    });
