...
class Popup extends Component {
  ...
  isDataValid(ids, orders) {
    ...
    for (let i = 0; i < selectedOrders.length; ++i) {
      ...
      if (parseFloat(selectedOrders[i].BALANCE_QUANTITY) < parseFloat(selectedOrders[i].QUANTITY)) {
        ...
        {isLast && (
          <div>
            ...
            <Dialog open={this.state.isDialogOpen} style={{ maxWidth: '1500px' }}>
              <AlertDialog closeAlert={() => this.closeAlert()} showAlert={this.state.showAlert} discardPO={() => this.discardPO()} />
              ...
              <DialogActions>
                <Button disabled={!this.isDataValid(this.props.selectedOrdersId, this.props.orders)} autoFocus onClick={() => { this.saveChanges(); }} color="primary">
                   Save
                </Button>
              </DialogActions>
            </Dialog>
